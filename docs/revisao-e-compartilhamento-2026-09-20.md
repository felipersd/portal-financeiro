...
Índices adicionados: lançamentos por usuário/data e usuário/recorrência/data, categorias e membros por usuário, além das buscas de convites e compartilhamentos. A estrutura usa React 19, Vite, Node 24, PostgreSQL e Prisma existentes; não adiciona servidores ou filas sem uma necessidade demonstrada.
...
A migração adiciona tabelas e índices, sem importar dados locais nem modificar tabelas existentes de forma destrutiva. Revisar o PR e os checks antes de integrar. A tag `v1.4.0` publica pelo fluxo já configurado. **Depois de usuários criarem compartilhamentos, não executar a imagem 1.3.0 sobre a mesma base:** ela não conhece os bloqueios de consentimento e pode alterar/excluir origens. Recuperação deve usar uma correção progressiva ou uma janela de manutenção com plano explícito.
