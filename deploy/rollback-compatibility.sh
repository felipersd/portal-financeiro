...
assert_rollback_compatible() {
  ...
  shift
  if [[ -f "$target/sharing-contract-version" ]] && [[ $(cat "$target/sharing-contract-version") == 1 ]]; then
    return 0
  fi
  if ! schema_present=$("$@" exec -T db psql -X -U portal -d finance_db -Atqc "SELECT to_regclass('finance.\"ExpenseShare\"') IS NOT NULL"); then
