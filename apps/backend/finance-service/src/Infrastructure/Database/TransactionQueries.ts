...
export class TransactionQueries {
    ...
    async page(userId: string, month: string, cursor?: string) {
        ...
        if (cursor) {
            try {
                ...
                if (
                    ...
                    !/^(share:)?[0-9a-f-]{36}$/i.test(decoded.id) ||
        ...
        const [owned, received] = await Promise.all([
            this.db.transaction.findMany({
                where: {
                    userId,
                    deletedAt: null,
                    id: { in: selected.filter((r) => !r.id.startsWith('share:')).map((r) => r.id) },
                },
            ...
            this.db.expenseShare.findMany({
                where: {
                    recipientId: userId,
                    status: 'accepted',
                    id: { in: selected.filter((r) => r.id.startsWith('share:')).map((r) => r.id.slice(6)) },
                },
            }),
        ]);
        ...
        return {
            items: selected.flatMap((r) => (items.has(r.id) ? [items.get(r.id)!] : [])),
            nextCursor:
                rows.length > 100 ? Buffer.from(JSON.stringify(selected[99])).toString('base64url') : null,
        };
    }
    ...
    async annual(userId: string, year: number) {
        ...
        const rows = await this.db.$queryRaw<
            Array<{ month: number; income: Prisma.Decimal; expense: Prisma.Decimal }>
        >`
