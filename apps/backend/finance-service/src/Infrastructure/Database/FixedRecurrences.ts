...
export class FixedRecurrences {
    ...
    async ensureYear(userId: string, year: number) {
        if (!Number.isInteger(year) || year < 1900 || year > 2199)
            throw new FinanceError(400, 'Ano inválido.');
        const rules = await this.db.fixedRule.findMany({
            where: {
                userId,
                anchorDate: { lt: new Date(Date.UTC(year + 1, 0, 1)) },
                OR: [{ endMonth: null }, { endMonth: { gt: `${year}-01` } }],
            },
            include: {
                transactions: {
                    where: { occurrenceMonth: { gte: `${year}-01`, lte: `${year}-12` } },
                    select: { occurrenceMonth: true },
                },
            },
        });
        const incomplete = rules.filter((rule) =>
            Array.from({ length: 12 }, (_, m) => `${year}-${String(m + 1).padStart(2, '0')}`).some(
                (month) =>
                    month >= rule.anchorDate.toISOString().slice(0, 7) &&
                    (!rule.endMonth || month < rule.endMonth) &&
                    !rule.transactions.some((t) => t.occurrenceMonth === month),
            ),
        );
                ...
                for (let m = 0; m < 12; m++) {
                    ...
                    if (
                        await tx.transaction.findUnique({
                            where: {
                                fixedRuleId_occurrenceMonth: { fixedRuleId: id, occurrenceMonth: month },
                            },
                            select: { id: true },
                        })
                    )
                    ...
                    const source = await tx.transaction.findFirst({
                        where: { fixedRuleId: id, occurrenceMonth: { lt: month } },
                        orderBy: { occurrenceMonth: 'desc' },
                        include: { splits: true, tags: true },
                    });
                    if (!source)
                        throw new FinanceError(
                            409,
                            'A recorrência precisa de revisão antes de gerar novas contas.',
                        );
    ...
    async stop(userId: string, transactionId: string) {
        await atomic(this.db, async (tx) => {
            ...
            if (!source?.isFixed || !source.recurrenceId)
                throw new FinanceError(404, 'Recorrência não encontrada.');
            const rule = source.fixedRuleId
                ? await tx.fixedRule.update({
                      where: { id: source.fixedRuleId },
            ...
            const future = rule
                ? { fixedRuleId: rule.id, occurrenceMonth: { gte: from } }
                : { userId, recurrenceId: source.recurrenceId, date: { gte: source.date } };
            if (
                await tx.expenseShare.count({
                    where: { transaction: future, status: { in: ['pending', 'accepted'] } },
                })
            ) {
                throw new FinanceError(
                    409,
                    'Há contas futuras pendentes ou aceitas. Cancele os envios pendentes e preserve os aceites antes de encerrar.',
                );
            }
