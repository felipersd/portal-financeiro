...
export const transactionSchema = z
    .object({
        ...
        splitDetails: z
            .object({
                splits: z
                    .array(z.object({ memberId: z.string().min(1).max(100), amount: money }))
                    .min(1)
                    .max(11),
            })
            .nullish(),
    ...
    .superRefine((data, ctx) => {
        if (data.isShared) {
            ...
            if (
                data.type !== 'expense' ||
                !splits.length ||
                new Set(splits.map((s) => s.memberId)).size !== splits.length ||
                ...
                ctx.addIssue({
                    ...
                    path: ['splitDetails'],
                    message:
                        'A divisão deve conter participantes únicos e somar exatamente o valor da despesa.',
                });
            }
            ...
            ctx.addIssue({
                code: 'custom',
                path: ['payer'],
                message: 'Uma conta individual deve ser paga por você e não pode conter divisão.',
            });
        }
    });
...
export const budgetSchema = z
    ...
    .superRefine((data, ctx) => {
        if (
            new Set(data.divisions.map((d) => d.id)).size !== data.divisions.length ||
        ...
        if (Object.values(data.mapping).some((id) => !ids.has(id)))
            ctx.addIssue({ code: 'custom', message: 'Selecione uma divisão válida para cada categoria.' });
    });
