...
suite('Sharing with real PostgreSQL', () => {
    ...
    beforeAll(() => {
        ...
        service = new SharingService(db);
        repository = new PrismaTransactionRepository(db);
        memberRepository = new PrismaGroupMemberRepository(db);
    });
    const owners: string[] = [];
    let owner: SharingActor,
        recipient: SharingActor,
        stranger: SharingActor,
        memberId: string,
    ...
    it('requires two consents, exposes only the recipient portion, and deduplicates retries', async () => {
        ...
        await expect(service.decideConnection(stranger, invite.id, 'accept')).rejects.toMatchObject({
            status: 404,
        });
        await service.decideConnection(recipient, invite.id, 'accept');
        const [first, duplicate] = await Promise.all([
            service.share(owner, transactionId, memberId),
            service.share(owner, transactionId, memberId),
        ]);
        expect(first.id).toBe(duplicate.id);
        expect(await repository.findByUserId(recipient.id, 2026)).toHaveLength(0);
        await expect(service.decideShare(stranger, first.id, 'accept')).rejects.toMatchObject({
            status: 404,
        });
        await Promise.all([
            service.decideShare(recipient, first.id, 'accept'),
            service.decideShare(recipient, first.id, 'accept'),
        ]);
    ...
    it('revokes pending shares', async () => {
        ...
        await expect(service.decideShare(recipient, share.id, 'accept')).rejects.toMatchObject({
            status: 409,
        });
    ...
    it('never books a declined expense and cannot resend it to bypass refusal', async () => {
        ...
        expect(await repository.findByUserId(recipient.id)).toHaveLength(0);
        expect(await service.share(owner, transactionId, memberId)).toMatchObject({
            id: share.id,
            status: 'declined',
        });
        await expect(service.decideShare(recipient, share.id, 'accept')).rejects.toMatchObject({
    ...
    it('rejects expired invitations and forged owner/member ids', async () => {
        ...
        await expect(service.decideConnection(recipient, invite.id, 'accept')).rejects.toMatchObject({
            status: 409,
        });
    ...
    it('requires the other person for adjustments and settlements and preserves the exact total', async () => {
        ...
        expect(
            source?.splitDetails.splits.find((s: { memberId: string }) => s.memberId === 'me').amount,
        ).toBe(70);
        expect(
            source?.splitDetails.splits.reduce((n: number, s: { amount: number }) => n + s.amount, 0),
        ).toBe(100);
        await expect(ledger.propose(recipient, share.id, 'payment', 2000)).rejects.toMatchObject({
            status: 400,
        });
        await expect(ledger.propose(owner, share.id, 'refund', 500)).rejects.toMatchObject({ status: 400 });
        const [next, same] = await Promise.all([
            ledger.propose(recipient, share.id, 'adjustment', 5000),
            ledger.propose(recipient, share.id, 'adjustment', 5000),
        ]);
        expect(next).toBe(same);
        await Promise.all([ledger.decide(owner, next, 'accept'), ledger.decide(owner, next, 'accept')]);
        expect((await db.expenseShare.findUniqueOrThrow({ where: { id: share.id } })).amountCents).toBe(5000);
        expect(
            (await repository.findById(transactionId))?.splitDetails.splits.reduce(
                (n: number, s: { amount: number }) => n + s.amount,
                0,
            ),
        ).toBe(100);
        await expect(ledger.history(stranger, share.id)).rejects.toMatchObject({ status: 404 });
        expect((await ledger.history(owner, share.id)).items).toHaveLength(2);
    });
    ...
    it('serializes concurrent revocation and acceptance', async () => {
        ...
        await Promise.allSettled([
            service.decideShare(recipient, share.id, 'accept'),
            service.decideConnection(owner, invite.id, 'revoke'),
        ]);
