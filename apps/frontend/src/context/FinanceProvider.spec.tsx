...
function Probe() {
    ...
    return (
        <div>
            ...
            <span data-testid="count">{finance.transactions.length}</span>
            ...
            <button onClick={() => void finance.removeTransaction('owned')}>Delete</button>
        </div>
    );
}
        ...
        vi.stubGlobal(
            ...
            vi.fn(async (url: string, options: RequestInit = {}) => {
                ...
                if (options.method === 'DELETE')
                    return new Response(JSON.stringify({ error: 'Conta aceita não pode ser excluída.' }), {
                        status: 409,
                    });
                let data: unknown = [];
                if (path === '/auth/me')
                    data = { id: auth.user.id, name: auth.user.id, email: `${auth.user.id}@example.test` };
                if (path.startsWith('/transactions/page'))
                    data = {
                        items: [
                            {
                                id: 'owned',
                ...
                if (path === '/preferences') data = { tagsEnabled: false };
                if (path === '/notifications') data = { items: [], unread: 0, nextCursor: null };
                if (path.startsWith('/budget-rules')) data = { divisions: [], mapping: {} };
                if (path.startsWith('/sharing')) data = { connections: [], shares: [] };
