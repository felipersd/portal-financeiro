...
export default defineConfig(({ mode, command }) => {
  ...
  if (command === 'build' && !(process.env.VITE_CLERK_PUBLISHABLE_KEY || env.VITE_CLERK_PUBLISHABLE_KEY)) {
    throw new Error('Configure VITE_CLERK_PUBLISHABLE_KEY antes de gerar o frontend de produção.');
  }
